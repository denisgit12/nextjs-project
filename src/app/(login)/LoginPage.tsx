...
import {SetLocalStore} from "@/app/helpers/LocalStore";
...
export default function LoginPage() {
    ...
    const [responses, setResponses] = useState<ZodIssue[] | []>([]);
    ...
    const onSubmit = async (e: FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            if (!response.success) {
                ...
                setErrors([]);
                setResponses([])
                SetLocalStore({name: 'email', value: email});
        ...
        <main className={"mainLoginForm"}>
            <form className="form" method="post" onSubmit={onSubmit}>
                ...
                <div className="flex-column">
                    {responses.length >= 1 && responses.map(value => value.path[0] == 'email')
                        ? responses.map((value, index) =>
                            <div key={index}>
                                {value.path[0] === 'email' && value.message}
                            </div>)
                        :
                        <label>Email </label>
                    }
                ...
                <div className="flex-column">
                    {responses.length >= 1 && responses.map(value => value.path[0] == 'password')
                        ? responses.map((value, index) =>
                            <div key={index}>
                                {value.path[0] === 'password' && value.message}
                ...
                <button onClick={() => setErrors([])} disabled={errors.length >= 1} className="button-submit">Sign In
                </button>
                <p className="p">Dont have an account? <span className="span">Sign Up</span></p>
