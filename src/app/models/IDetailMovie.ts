export interface IDetailMovie {
 ...
 interface RootObjectBelongs_to_collection {
	id: number;
	name: string;
	poster_path: string;
	backdrop_path: string;
}
 interface RootObjectGenres {
	id: number;
	name: string;
}
interface RootObjectProduction_companies {
	id: number;
	logo_path: string;
	name: string;
	origin_country: string;
}
 interface RootObjectProduction_countries {
	iso_3166_1: string;
	name: string;
}
interface RootObjectSpoken_languages {
	english_name: string;
	iso_639_1: string;
	name: string;
}